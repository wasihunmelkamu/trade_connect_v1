...
export const signInAction2 = async (
    { password, hashedPassword, userId, role }:
        { userId: Id<"users">, role: "admin" | "user", password: string, hashedPassword: string, }
) =>
    serverActionWrapper(async () => {
...
export const verifyUser = async () => {
    return serverActionWrapper(async () => {
        ...
        const user = await fetchQuery(api.users.getUser, { userId: payload.userId as Id<"users"> })
